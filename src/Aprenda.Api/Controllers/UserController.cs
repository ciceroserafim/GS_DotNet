...
namespace Aprenda.Api.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<ActionResult<object>> Login([FromBody] UsuarioDto dto)
        {
            ...
            var token = JwtTokenGenerator.GenerateToken(user, _jwtConfig);
            ...
            var response = new
            {
                ...
                Token = token,
                TokenType = "Bearer",
                ExpiresIn = _jwtConfig.ExpiresMinutes * 60,
                Links = new[]
                {
                    new { rel = "self", href = Url.Action(nameof(Get), new { id = user.Id, version = "1.0" }) },
                    new { rel = "profile", href = Url.Action(nameof(Get), new { id = user.Id, version = "1.0" }) }
                }
            };
